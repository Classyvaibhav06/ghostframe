...
export default function SignUpPage() {
  ...
  return (
    ...
    >
      ...
      {/* Decorative blobs */}
      <div className="absolute top-24 -right-16 w-64 h-64 bg-[var(--color-highlighter-yellow)] rounded-full blur-3xl opacity-30 pointer-events-none" />
      <div className="absolute bottom-16 -left-16 w-48 h-48 bg-[var(--color-sticky-note-mint)] rounded-full blur-3xl opacity-40 pointer-events-none" />
      ...
      >
        ...
        <div className="flex justify-center mb-4">
          <span className="bg-[var(--color-highlighter-yellow)] text-[var(--color-forest-ink)] text-[12px] font-mono font-bold uppercase tracking-widest px-4 py-1.5 rounded-full border border-[var(--color-forest-ink)]/20 shadow-[2px_2px_0_rgba(0,0,0,0.08)]">
            ✦ 5 free videos + 25 free images included
          </span>
        </div>
        ...
        <div className="bg-white border-2 border-[var(--color-forest-ink)] rounded-[8px] shadow-[8px_8px_0px_0px_var(--color-forest-ink)] p-8 sm:p-10 relative overflow-hidden">
          ...
          <h1 className="font-display text-[36px] sm:text-[42px] leading-tight text-[var(--color-forest-ink)] mb-1 relative inline-block">
            Create Account
            <div className="absolute bottom-1 left-0 w-full h-3 bg-[var(--color-sticky-note-mint)] -z-10 transform -rotate-1" />
          </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#8a9c7b] hover:bg-[#7b8e6c] text-[var(--color-cream-paper)] font-bold text-[16px] rounded-[6px] py-[14px] px-6 transition-all flex items-center justify-center gap-2 shadow-[4px_4px_0px_0px_var(--color-forest-ink)] hover:shadow-[2px_2px_0px_0px_var(--color-forest-ink)] hover:translate-x-[2px] hover:translate-y-[2px] disabled:opacity-50 disabled:cursor-not-allowed mt-2"
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Start for Free →'}
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-dashed border-[var(--color-pencil-gray)]/50 text-center">
            <p className="text-[14px] text-[var(--color-forest-ink)] opacity-70 font-mono">
              ...
              <Link to="/signin" className="font-bold text-[var(--color-forest-ink)] underline underline-offset-2 hover:opacity-100 transition-opacity">
                Sign in here
              </Link>
            </p>
          </div>
